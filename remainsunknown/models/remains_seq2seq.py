...
class RemainsSeq2Seq(Model):
    """
    This ``RemainsSeq2Seq`` class is a :class:`Model` which takes a sequence, encodes it, and then
    uses the encoded representations to decode another sequence.  This is derived from the RemainsSeq2Seq model.
    Parameters
    ----------
    ...
    target_namespace : ``str``, optional (default = 'remains')
    ...
    def __init__(self,
        ...
        super(RemainsSeq2Seq, self).__init__(vocab)
    ...
    def forward(self,  # type: ignore
                ...
                remains: Dict[str, torch.LongTensor] = None) -> Dict[str, torch.Tensor]:
        ...
        remains : ``Dict[str, torch.LongTensor]``, optional (default = None)
        ...
        """
        state = self._encode(abstract)

        if remains:
            state = self._init_decoder_state(state)
            # The `_forward_loop` decodes the input sequence and computes the loss during training
            # and validation.
            output_dict = self._forward_loop(state, remains)
        else:
            output_dict = {}
        ...
        if not self.training:
            ...
            if remains and self._bleu:
                # shape: (batch_size, beam_size, max_sequence_length)
                top_k_predictions = output_dict["predictions"]
                # shape: (batch_size, max_predicted_sequence_length)
                best_predictions = top_k_predictions[:, 0, :]
                self._bleu(best_predictions, remains["tokens"])
    ...
    def _encode(self, abstract: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        # shape: (batch_size, max_input_sequence_length, encoder_input_dim)
        embedded_input = self._source_embedder(abstract)
        # shape: (batch_size, max_input_sequence_length)
        source_mask = util.get_text_field_mask(abstract)
    ...
    def _forward_loop(self,
        ...
        if remains:
            ...
            # Compute loss.
            target_mask = util.get_text_field_mask(remains)
